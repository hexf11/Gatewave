...
- TCP 使用 O(1) 空闲时间轮、8–128 KiB 自适应 DirectBuffer、同事件直写、有界公平预算、背压与半关闭排空窗口。
- TCP 在连接集中爆发时动态缩小单连接预算；每个方向前 256 KiB 使用独立交互流量池，多客户端按 Selector 轮次获得等额字节份额。
...
- 提供峰值、活跃设备、DNS、建连 P50/P95、实时吞吐、TCP 排队/重调度 P50/P95、交互直写/公平让出/背压/缓冲复用、UDP 快路径/解析/队列峰值/丢弃、公平回收与 Selector 指标。
